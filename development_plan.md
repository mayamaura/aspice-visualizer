...
### 受け入れ条件
- [x] ビュー切替・パネル開閉に滑らかなトランジションが付く
- [x] `prefers-reduced-motion: reduce` でアニメーションが無効になる
- [x] PNG エクスポート時にトーストが表示される
- [x] 狭幅ウィンドウでヘッダーが破綻しない（タブがアイコン化 or 折返し）
- [x] アイコンのみボタンに `aria-label` がある（スクリーンリーダ読み上げ確認）
- [x] Tab キーでフォーカス移動でき、フォーカスリングが見える
- [x] モーダルが `role="dialog"` を持ち、Esc で閉じ、フォーカスが内側に留まる
